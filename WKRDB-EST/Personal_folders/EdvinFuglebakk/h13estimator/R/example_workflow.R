...
compare_with_eca <- function() {
  ...
  ecaPath <-
    system.file("exampledata", "eca_2018.csv", package = "h13estimator")
  ecaResults <-
    read.csv(ecaPath, sep = "\t", comment.char = "#") #other reports and plusgroup is commented out in file
  ...
  legend(
    "topright",
    legend = c("Design based", "Model based (ECA)"),
    fill = c("black", "red"),
    bty = "n"
  )
}
