applyGenerateProbs <- function(x, probType, overwrite, runInitialProbChecks = TRUE){
...
if(length(unique(x[["DE"]]$hierarchy))>1) stop (">1 hierarchy in data: not yet developed")
...
# following code will be worth setting in data.table
	...
	ls2 <- lapply(ls1, function(x, ...) {
				# aspects needing development
					if(length(unique(x$stratumName))>1 | any(x$stratification=="Y")) stop ("stratification present: not yet developed")
					if(length(unique(x$clusterName))>1 | any(x$clustering=="Y")) stop ("clustering present: not yet developed")
				print(paste0(parentId[target_tables==i],": ", x[[parentId[target_tables==i]]][1])) # convenient to correct errors
