...
getLowerProbs <- function(table, hierarchyType = "A", BVtype = "weight", probType = "inclusion") {
  ...
  if (tolower(probType) == "selection")
  {
    if( uniqueSelMethod %in% c("SRSWR" , "SRSWOR") )
    {
      if( uniqueSelMethod == "SRSWR") {
        if (nrow(myBV[myBV$BVnumTotal <0,]))
        {
          stop ("Zeros in BVnumTotal - can't proceed")
        }
        myBV$prob <-  1/myBV$BVnumTotal
      }
  ...
  else if (tolower(probType) == "inclusion")
  {
    if(length(uniqueSelMethod)>1) { 
      ...
      if( uniqueSelMethod %in% c("SRSWR" , "SRSWOR") )
      {
        if(sum(is.na(myBV$BVnumTotal))>0) stop ("Cannot proceed: NAs in total")
        if(sum(is.na(myBV$BVnumSamp))>0) stop ("Cannot proceed: NAs in sampled")
        if( uniqueSelMethod == "SRSWR") {
          if (nrow(myBV[myBV$BVnumTotal <0,]))
          {
            stop ("Cannot proceed: Zeros in BVnumTotal")
          }
          myBV$prob <-  1-(1-1/myBV$BVnumTotal)^myBV$BVnumSamp
        }
        if( uniqueSelMethod == "SRSWOR"){
          ...
          myBV$prob <-  myBV$BVnumSamp/myBV$BVnumTotal
        }
      }
      if( uniqueSelMethod %in% c("UPSWR" , "UPSWOR"))
      {
  ...
  rownames(myBVDesignTable) <- myBVDesignTable$id
